...
#####
#agregamos pesos a nuestro modelos
# Crear matriz de predictores y vector de respuesta para el conjunto de entrenamiento
x_trainp <- dvf_train[, -which(names(dvf_train) == "Pobre.1")]
y_trainp <- dvf_train$Pobre.1
x_testp <- dvf_test

#contruimos los pesos
pesos <- rep(1, nrow(dvf_train))
pesos[dvf_train$Pobre.1 ==1] <- 4
pesos[dvf_train$Pobre.1 ==0] <- 2

modelo7<- train(x =  select(dvf_train, -Pobre.1),
                y= as.factor(dvf_train$Pobre.1),
                preProcess=NULL,
                method = "glmnet",
                weights = pesos)
#predecimos 
insamplep<- predict(modelo7, dvf_train)

#analisamos su acc
acc_insamplep <- Accuracy(y_pred= insamplep, y_true= train$Pobre.1)
acc_insamplep 

#analisamos precision
pre_insamplep <- Precision(y_pred= insamplep, y_true= train$Pobre.1, positive = 1)
pre_insamplep 

#analisamos recall
rec_insamplep <- Recall(y_pred= insamplep, y_true= train$Pobre.1, positive = 1)
rec_insamplep 

#analisamos f1
f1_insamplep <- F1_Score(y_pred= insamplep, y_true= train$Pobre.1, positive = 1)
f1_insamplep 

metricas_insample3 <- data.frame(Modelo = "Regresión logistica Elastic Net", 
                                 "Muestreo" = NA, 
                                 "Evaluación" = "Dentro de muestra",
                                 "Accuracy" = acc_insamplep,
                                 "Precision - PPV" = pre_insamplep,
                                 "Recall - TPR - Sensitivity" = rec_insamplep,
                                 "F1" = f1_insamplep)

# modelo con los datos balanceados y pesos 
