...
df_training_hogares1<-df_training_personas %>% group_by(id) %>% summarize(Nro_mujeres=sum(female,na.rm = TRUE),
...
df_training_hogares <- df_hogares %>% mutate(Indigente=factor(Indigente,levels=c(1,0)))
